...
const unboxCheckGetBoxDataData = (data) => {
    ...
    } else if(data.boxId && typeof data.boxId === 'string' && validator.isMongoId(data.boxId) !== true && !data.slug) {
        throw new Error('You’ve entered an invalid box id.');
    }
}
...
const unboxGetOutcomeItem = (boxDatabase, outcome) => {
    ...
    for(const entry of boxDatabase.items) {
        if(entry.minTicket !== undefined && entry.maxTicket !== undefined && outcome >= entry.minTicket && outcome <= entry.maxTicket) {
            outcomeItem = entry.item && entry.item.amountFixed !== undefined ? entry.item : {
                name: entry.name,
                image: entry.image,
                amountFixed: entry.amountFixed,
                color: entry.color,
                dropId: entry.dropId,
                minTicket: entry.minTicket,
                maxTicket: entry.maxTicket
            };
            break;
        }
    }
