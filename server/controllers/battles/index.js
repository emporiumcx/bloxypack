...
const battlesGameValidate = async(io, battlesGame) => {
    try {
        ...
        let seedPublic = crypto.randomBytes(32).toString('hex');
        let blockId = Date.now();
        try {
            const dataFair = await fairGetData();
            if (dataFair && dataFair.data && dataFair.data.head_block_id) {
                seedPublic = dataFair.data.head_block_id;
                blockId = dataFair.data.head_block_num;
            }
        } catch (err) {}

        battlesGame.fair.seedPublic = seedPublic;
        battlesGame.fair.blockId = blockId;
