...
const towersSendBetSocket = async(io, socket, user, data, callback) => {
    try {
        ...
        // Get users towers game
        const towersGame = towersGames.find((element) => socketRefId(element.user) === socketRefId(user._id));
        ...
        let deck = towersGenerateDeck(data.risk, seedDatabase);
...
const towersSendRevealSocket = async(io, socket, user, data, callback) => {
    try {
        ...
        // Get users towers game
        let towersGame = towersGames.find((element) => socketRefId(element.user) === socketRefId(user._id));
        ...
        if(towersGame.revealed[towersGame.revealed.length - 1].row[tile] === 'lose' || towersGame.revealed.length >= 10) {
