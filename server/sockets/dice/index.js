...
const { diceSendBetSocket } = require('../../controllers/dice');

module.exports = (io) => {
    io.of('/dice').use(async (socket, next) => {
        try {
            const identifier = socket.handshake.headers['cf-connecting-ip'] || socket.conn.remoteAddress;
            await socketCheckConnectionLimit('dice', identifier);
            try {
    ...
    io.of('/dice').on('connection', (socket) => {
        const identifier = socket.handshake.headers['cf-connecting-ip'] || socket.conn.remoteAddress;
        socketAddConnectionLimit('dice', identifier);
            ...
            if (socket.decoded !== undefined && socket.decoded !== null) {
                try {
                    ...
                    try {
                        ...
                        try { settingCheck(user, 'games.dice.enabled'); } catch (e) { settingCheck(user, 'games.roll.enabled'); }
                        diceSendBetSocket(io, socket, user, data, callback);
                    } catch (err) {
        ...
        socket.on('disconnect', () => {
            socketRemoveConnectionLimit('dice', identifier);
        });
    });
};
