...
const {
    ...
    blackjackSoloStand,
    blackjackSoloDouble
} = require('../../controllers/blackjack/solo');
...
module.exports = (io) => {
    ...
    io.of('/blackjack').on('connection', (socket) => {
        ...
        const soloHandler = (event, handler) => {
                ...
                if(socket.decoded !== undefined && socket.decoded !== null) {
                    try {
                        ...
                        try {
                            ...
                            socketCheckUserData(user, true);
                            settingCheck(user, 'games.blackjack.enabled');
                            handler(io, socket, user, data, callback);
                        } catch(err) {
                            socketRemoveAntiSpam(socket.decoded._id);
                            callback({ success: false, error: { type: 'error', message: err.message } });
                        }
        ...
        soloHandler('sendSoloBet', blackjackSoloBet);
        soloHandler('sendSoloHit', blackjackSoloHit);
        soloHandler('sendSoloStand', blackjackSoloStand);
        soloHandler('sendSoloDouble', blackjackSoloDouble);
