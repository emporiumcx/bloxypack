...
const minesSendBetSocket = async(io, socket, user, data, callback) => {
    try {
        ...
        // Get users mines game
        const minesGame = minesGames.find((element) => socketRefId(element.user) === socketRefId(user._id));
        ...
        const grid = Math.floor(data.grid || 5);
        ...
        const deck = minesGetGameDeck(minesCount, grid, seedDatabase);
...
const minesSendRevealSocket = async(io, socket, user, data, callback) => {
    try {
        ...
        // Get users mines game
        let minesGame = minesGames.find((element) => socketRefId(element.user) === socketRefId(user._id));
        ...
        const cells = (minesGame.grid || 5) * (minesGame.grid || 5);
        const safeRevealed = minesGame.revealed.filter((element) => element.value === 'coin').length;

        if(minesGame.deck[tile] === 'mine' || safeRevealed >= (cells - minesGame.minesCount)) {
            // Get payout amount
