    ...
    try {
        ...
        if(settings === undefined || settings === null) {
            settings = await Setting.create({
                ...
                crypto: {
                    deposit: { enabled: false, rate: 2.1 },
                    withdraw: { enabled: false, rate: 2 }
                },
                gift: {
                    deposit: { enabled: false },
                    withdraw: { enabled: false }
...
const settingCheck = (user, value) => {
    if(settings.general.maintenance.enabled === true && (user === null || user.rank !== 'admin')) {
        ...
        const found = value.split('.').reduce((o, i) => (o == null ? undefined : o[i]), settings);
        if(found === false && (user === null || user.rank !== 'admin')) {
